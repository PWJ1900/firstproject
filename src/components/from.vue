 ...
 <el-dialog title="编辑学生信息" :visible="editstudentForm" size="tiny" :modal-append-to-body='false' @close='closeDialog'>
   <el-form ref="editsForm" :model="editsForm" label-width="80px">
     <el-form-item label="title">
       <el-input  v-model="editsForm.title" max-length="10" disabled="disabled"></el-input>
     </el-form-item>
  ...
  <el-button
          size="mini"
          type="danger"
          @click="handleConfirm()">Add</el-button>
  </el-row>
</template>
...
<script>
  export default {
    ...
    data() {
       this.$http.get('http://127.0.0.1:8000/blog/articleuse').then(response => {
     ...
     methods:{
      ...
      studentAdd(){
         let tran = this.addsForm
           this.$http.post('http://127.0.0.1:8000/blog/articleuse',tran).then(response=>{
      ...
      studentcEdit(){
        ...
        this.$http.put('http://127.0.0.1:8000/blog/articleuse',tran).then(response=>{
