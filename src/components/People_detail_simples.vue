 ...
 <el-dialog
  ...
  <span slot="footer" class="dialog-footer">
    ...
    <el-col v-show="shownow1">
      ...
      ></el-input>
      姓名:<el-input
        v-model="addsForm.name"
        style="width: 100px; height: 20px"
        max-length="10"
      ></el-input>
      身份:
      <el-input
        v-model="addsForm.identity"
        style="width: 100px; height: 20px"
      ></el-input>
      major:
      <el-input
        v-model="addsForm.major"
        style="width: 100px; height: 20px"
      ></el-input>
      subject:
      <el-input
        v-model="addsForm.subject"
    ...
    <el-col v-show="shownow2">
      ...
      身份:
      <el-input
        v-model="editsForm.identity"
        style="width: 100px; height: 17px"
      ></el-input>
      major:<el-input
        v-model="editsForm.major"
        style="width: 100px; height: 17px"
      ></el-input>
      subject:
      <el-input
        v-model="editsForm.subject"
...
export default {
  ...
  methods: {
            ...
            (response) => {
              ...
              this.tableData = this.tableData.filter(
                (data) =>
                  ...
                  data.name.toLowerCase().includes(datasearch.toLowerCase()) ||
                  (data.identity + "")
                    .toLowerCase()
                    .includes(datasearch.toLowerCase())
              );
    ...
    deleteju() {
      this.$http
        .post("http://127.0.0.1:8000/blog/people_simple", this.deletetran)
        .then(
          (response) => {
            this.$http
              .get("http://127.0.0.1:8000/blog/people_simple", {
    ...
    studentAdd() {
      ...
      tran["num"] = "2";
      this.$http.post("http://127.0.0.1:8000/blog/people_simple", tran).then(
        (response) => {
          this.$http
            .get("http://127.0.0.1:8000/blog/people_simple", {
              params: { pageNum: this.pagenumuse },
