...
<el-form id="#addsForm" ref="addsForm" :model="addsForm" label-width="80px">
     ...
     <el-form-item label="slider_title" prop="slider_title">
       ...
       <el-input type="textarea" :rows="6" v-model="addsForm.slider_description"></el-input>
     </el-form-item>
     <el-form-item>
 ...
 <el-dialog title="编辑学生信息" :visible="editstudentForm" size="tiny" :modal-append-to-body='false' @close='closeDialog'>
   <el-form ref="editsForm" :model="editsForm" label-width="80px">
     <el-form-item label="slider_image">
       <el-input  v-model="editsForm.slider_image" max-length="10" disabled="disabled"></el-input>
     </el-form-item>
     <el-form-item label="slider_title">
       <el-input v-model="editsForm.slider_title"></el-input>
     </el-form-item>
     <el-form-item label="slider_description">
       <el-input type="textarea" :rows="6" v-model="editsForm.slider_description"></el-input>
    </el-form-item>
...
<script scoped>
  export default {
    ...
    data() {
       this.$http.get('http://127.0.0.1:8000/blog/index_slider',{params:{pageNum:1}}).then(response => {
  ...
  // error callback
     ...
     methods:{
      ...
      handleCurrentChange(val) {
        ...
        this.$http.get("http://127.0.0.1:8000/blog/index_slider",{params:{pageNum:this.pagenumuse}}).then(
      (response) => {
        console.log(response);
        let a = response.data;
        this.totalget = response.data[0]['count']
      ...
      deleteju(){
         this.$http.post('http://127.0.0.1:8000/blog/index_slider',this.deletetran).then(response=>{
          this.$http.get("http://127.0.0.1:8000/blog/index_slider",{params:{pageNum:this.pagenumuse}}).then(
            (response) => {
              console.log(response);
              let a = response.data;
      ...
      if(datasearch!=''){
        this.$http.get("http://127.0.0.1:8000/blog/index_slider",{params:{pageNum:''}}).then((response)=>{
          this.tableData = response.data 
          this.tableData = this.tableData.filter(data => !datasearch || (data.pk+'').toLowerCase().includes(datasearch.toLowerCase()) || data.slider_title.toLowerCase().includes(datasearch.toLowerCase())||(data.slider_description+'').toLowerCase().includes(datasearch.toLowerCase()))},(response)=>{})

      }},
     studentAdd(){
         let tran = this.addsForm
         tran['num'] = '2'
           this.$http.post('http://127.0.0.1:8000/blog/index_slider',tran).then(response=>{
              this.$http.get('http://127.0.0.1:8000/blog/index_slider',{params:{pageNum:this.pagenumuse}}).then(response => {
        console.log(response)
         this.totalget = response.data[0]['count']
        let a = response.data
        console.log(a)
