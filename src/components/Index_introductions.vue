<template>
  <el-row>
    ...
    <el-col v-show="shownow2"
      ...
      <vue-ueditor-wrap
        v-model="editsForm.introduction"
        :config="myConfig"
      ></vue-ueditor-wrap>
    </el-col>
  </el-row>
...
export default {
  ...
  data() {
    this.$http
      .get("http://127.0.0.1:8000/blog/introduction", {
        params: { pageNum: 1 },
      })
      .then(
        (response) => {
  ...
  methods: {
    ...
    deleteju() {
      this.$http
        .post("http://127.0.0.1:8000/blog/introduction", this.deletetran)
        .then(
          (response) => {
            this.$http
              .get("http://127.0.0.1:8000/blog/introduction", {
    ...
    studentAdd() {
      ...
      tran["num"] = "2";
      this.$http.post("http://127.0.0.1:8000/blog/introduction", tran).then(
        (response) => {
          this.$http
            .get("http://127.0.0.1:8000/blog/introduction", {
              params: { pageNum: this.pagenumuse },
