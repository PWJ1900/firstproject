<template>
  <el-row>
    ...
    >
      ...
      <el-table-column prop="pic" label="个人照片" width="150">
        <template slot-scope="scope">
      ...
      <el-table-column prop="category" label="类型号" width="120">
        <template slot-scope="scope">
...
export default {
  ...
  data() {
    this.$http
      .get("http://127.0.0.1:8000/blog/peopleuse", { params: { pageNum: 1 } })
      .then(
        (response) => {
  ...
  methods: {
    ...
    imgBroadcastChange2(file) {
      ...
      if (isLt2M) {
        ...
        Usebs4(file.raw).then((data) => {
          this.editsForm.pic = data.result;
        });
      } else {
        this.$message.error("上传封面图片大小不能超过 2GB!");
      }
    ...
    deleteju() {
      this.$http
        .post("http://127.0.0.1:8000/blog/peopleuse", this.deletetran)
        .then(
          (response) => {
            this.$http
              .get("http://127.0.0.1:8000/blog/peopleuse", {
    ...
    studentAdd() {
      ...
      tran["num"] = "2";
      this.$http.post("http://127.0.0.1:8000/blog/peopleuse", tran).then(
        (response) => {
          this.$http
            .get("http://127.0.0.1:8000/blog/peopleuse", {
              params: { pageNum: this.pagenumuse },
