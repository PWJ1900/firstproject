    ...
    >
      <el-form ref="editsForm" :model="editsForm" label-width="80px">
        ...
        <el-form-item label="项目名字:">
          <el-input v-model="editsForm.projectname" max-length="10"></el-input>
        </el-form-item>
  ...
  methods: {
    ...
    deleteju() {
      this.$http.post(config.requesturl + "/project_vue", this.deletetran).then(
        (response) => {
          this.$http
            .get("http://127.0.0.1:8000/blog/project_vue", {
    ...
    searchinfo(datasearch) {
      if (datasearch != "") {
        this.$http
          .get(config.requesturl + "/project_vue", { params: { pageNum: "" } })
          .then(
            (response) => {
