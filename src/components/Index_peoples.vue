<template>
  <el-row>
    ...
    >
      ...
      <el-table-column prop="tutor_en" label="tutor_en" width="120">
      </el-table-column>
...
export default {
  ...
  data() {
    this.$http
      .get("http://127.0.0.1:8000/blog/index_people", {
        params: { pageNum: 1 },
      })
      .then(
        (response) => {
  ...
  methods: {
    imgBroadcastChange(file) {
      ...
      if (isLt2M) {
        ...
        Usebs4(file.raw).then((data) => {
          this.addsForm.people_pic = data.result;
        }); //data就是usebs4转化过的值
      } else {
            ...
            (response) => {
              ...
              this.tableData = this.tableData.filter(
                (data) =>
                  ...
                  data.tutor.toLowerCase().includes(datasearch.toLowerCase()) ||
                  data.tutor_en.toLowerCase().includes(datasearch.toLowerCase())
              );
            },
            (response) => {}
          );
    ...
    imgBroadcastChange2(file) {
      ...
      if (isLt2M) {
        ...
        Usebs4(file.raw).then((data) => {
          this.editsForm.people_pic = data.result;
        });
      } else {
        this.$message.error("上传封面图片大小不能超过 2GB!");
      }
    ...
    deleteju() {
      this.$http
        .post("http://127.0.0.1:8000/blog/index_people", this.deletetran)
        .then(
          (response) => {
            this.$http
              .get("http://127.0.0.1:8000/blog/index_people", {
    ...
    studentAdd() {
      ...
      tran["num"] = "2";
      this.$http.post("http://127.0.0.1:8000/blog/index_people", tran).then(
        (response) => {
          this.$http
            .get("http://127.0.0.1:8000/blog/index_people", {
              params: { pageNum: this.pagenumuse },
