<template>
  <el-row>
    ...
    >
      ...
      <el-table-column prop="university" label="大学" width="120">
      </el-table-column>
...
export default {
  ...
  data() {
    this.$http
      .get("http://127.0.0.1:8000/blog/education", { params: { pageNum: 1 } })
      .then(
        (response) => {
  ...
  methods: {
    ...
    searchinfo(datasearch) {
      if (datasearch != "") {
        this.$http
          .get("http://127.0.0.1:8000/blog/education", {
            params: { pageNum: "" },
          })
          .then(
            (response) => {
    ...
    deleteju() {
      this.$http
        .post("http://127.0.0.1:8000/blog/education", this.deletetran)
        .then(
          (response) => {
            this.$http
              .get("http://127.0.0.1:8000/blog/education", {
    ...
    studentAdd() {
      ...
      tran["num"] = "2";
      this.$http.post("http://127.0.0.1:8000/blog/education", tran).then(
        (response) => {
          this.$http
            .get("http://127.0.0.1:8000/blog/education", {
              params: { pageNum: this.pagenumuse },
